...
from rig.machine_control.consts import SCPCommands, SCPReturnCodes, SCP_PORT, FATAL_SCP_RETURN_CODES
...
from _rig_c_scp.lib import *


class CSCPConnection(Thread):
    ...
    made from any thread and the requests are initially queued (in
    ...
    def __init__(self, hostname, port=SCP_PORT, n_tries=5, timeout=0.5,
                 n_outstanding=1, scp_data_length=256):
        super(CSCPConnection, self).__init__(
            name="CSCPConnection to {}:{}".format(hostname, port))
        ...
        self._addrinfo = hostname_to_addrinfo(hostname.encode("UTF-8"),
                                              str(port).encode("ASCII"))
        if self._addrinfo == ffi.NULL:
            raise IOError("Could not resolve {}.".format(hostname))
        self._addrinfo = ffi.gc(self._addrinfo, free)
        ...
        self._loop = malloc_uv_loop_t()
        assert self._loop != ffi.NULL, \
            "Failed to allocate memory for libuv event loop"
        self._loop = ffi.gc(self._loop, free)
        assert uv_loop_init(self._loop) == 0, \
    ...
    def run(self):
        """Background thread in which the main LibUV event loop will run."""
    ...
    def _rs_free(self):
        ...
        rs_free(self._conn, lib.free_cb, self._self)
        self._conn = None
    ...
    def _wakeup(self):
        """Wakeup the libuv thread."""
        uv_async_send(self._wakeup_handle)
    
    def _on_wakeup(self):
        """Callback called from the libuv thread some time after _wakeup has
        been called.
        """
        ...
        # If the connection is closed, kick off any/all remaining queued
        # actions and free the connection (causing the queued actions to have
        # their error callback called).
        with self._lock:
            if self._closed:
                while self._queue:
                    self._queue.popleft()()
    ...
    def _report_error(self, error, cmd_rc, callback):
        ...
        if error:
            ...
            if callback.on_error is not None:
                err_name = ffi.string(rs_err_name(error)).decode("ASCII")
                if err_name == "RS_EBAD_RC":
                    callback.on_error(FatalReturnCodeError(cmd_rc, callback.packet))
                elif err_name == "RS_ETIMEOUT":
                    callback.on_error(TimeoutError(packet=callback.packet))
                else:
                    callback.on_error(SCPError(
                        "{}: {}".format(err_name, ffi.string(rs_strerror(error)).decode("ASCII")),
                        callback.packet))
            return True
    ...
    def send_scp(self, x, y, p, cmd, arg1=0, arg2=0, arg3=0, data=b'',
        ...
        data_buf.base = malloc(self._scp_data_length)
        ...
        rs_send_scp(self._conn,
                    (((x & 0xFF) << 8) | (y & 0xFF)),
                    p,
                    cmd,
                    3, # n_args_send always three...
                    expected_args,
                    arg1, arg2, arg3,
                    data_buf[0],
                    self._scp_data_length,
    ...
    def _on_scp_cb(self, error, cmd_rc, n_args, arg1, arg2, arg3, data_buf,
        ...
        if not self._report_error(error, cmd_rc, callback):
            if callback.on_success is not None:
                ...
                callback.on_success(SCPPacket(cmd_rc=cmd_rc,
    ...
    @_execute_in_bg_thread
    def read(self, x, y, p, address, length, buffer=None, on_success=None, on_error=None):
    ...
    def _on_read_cb(self, error, cmd_rc, data_buf, callback):
        ...
        if not self._report_error(error, cmd_rc, callback):
            if callback.on_success is not None:
                # Create a buffer if the user didn't provide one
                buffer = callback.buffer
                if buffer is None:
                    buffer = bytearray(data_buf.len)
                
                # Copy read data into a Python-accessible buffer
                ffi.memmove(buffer, data_buf.base, min(data_buf.len, len(buffer)))
                
                callback.on_success(buffer)
